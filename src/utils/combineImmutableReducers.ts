...
function getUnexpectedStateKeyWarningMessage (inputState: AnyMap, outputState: AnyMap, action: Action) {
  ...
  if (reducerKeys.length === 0) {
    return (
      ...
      'to combineImmutableReducers is an object whose values are reducers.'
    )
  }
...
function assertReducerSanity (reducers: {[s: string]: Reducer<any>}) {
  Object.keys(reducers).forEach((key) => {
    ...
    const type = `${ActionTypes.UNKNOWN}_${Math.random().toString(36).substring(7).split('').join('.')}`
...
export default function combineImmutableReducers (
  ...
  return function combination (state = defaultState, action: Action) {
    ...
    const finalState = mapValues(finalReducers, (reducer, key) => {
      const previousStateForKey = isImmutable(state) ? state.get(key) : undefined
    ...
    if (process.env.NODE_ENV !== 'production') {
      const warningMessage = getUnexpectedStateKeyWarningMessage(state, finalState, action)
      if (warningMessage) {
